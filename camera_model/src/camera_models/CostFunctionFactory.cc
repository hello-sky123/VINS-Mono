...
template<typename T> // 这里的worldToCameraTransform表示相机坐标系到世界坐标系的变换，可以将世界坐标系下的点变换到相机坐标系下
// q_cam_odo: 相机坐标系到里程计坐标系的旋转，里程计的姿态惯性系变换到里程计坐标系
void worldToCameraTransform(const T* const q_cam_odo, const T* const t_cam_odo, const T* const p_odo, const T* const att_odo,
                            T* q, T* t, bool optimize_cam_odo_z = true) {
  Eigen::Quaternion<T> q_z_inv(cos(att_odo[0] / T(2)), T(0), T(0), -sin(att_odo[0] / T(2))); // sin(theta/2) * 单位方向向量
  Eigen::Quaternion<T> q_y_inv(cos(att_odo[1] / T(2)), T(0), -sin(att_odo[1] / T(2)), T(0)); // 绕z的旋转可以用轴角表示，然后轴角转换成四元数
  Eigen::Quaternion<T> q_x_inv(cos(att_odo[2] / T(2)), -sin(att_odo[2] / T(2)), T(0), T(0)); // [cos(theta/2), sin(theta/2) * 单位方向向量]

  Eigen::Quaternion<T> q_zyx_inv = q_x_inv * q_y_inv * q_z_inv;

  T q_odo[4] = {q_zyx_inv.w(), q_zyx_inv.x(), q_zyx_inv.y(), q_zyx_inv.z()};  // 因为前面求了逆，所以变成了里程计坐标系到惯性系的旋转
                                                         // q_cam_odo四元数顺序是x,y,z,w(Eigen默认顺序)，q_odo_cam四元数顺序是w,x,y,z(Ceres默认顺序)
  T q_odo_cam[4] = {q_cam_odo[3], -q_cam_odo[0], -q_cam_odo[1], -q_cam_odo[2]}; // 输入的q_cam_odo是odo到cam的旋转，这里转换成cam到odo的旋转

  T q0[4];
  ceres::QuaternionProduct(q_odo_cam, q_odo, q0); // c->o * o->w = c->w

  T t0[3];
  T t_odo[3] = {p_odo[0], p_odo[1], p_odo[2]};

  ceres::QuaternionRotatePoint(q_odo, t_odo, t0); // 将惯性系下的点变换到里程计坐标系下，t_{w->o}^{o}

  t0[0] += t_cam_odo[0]; // t_{w->o}^{o} + t_{o->c}^{o} = t_{w->c}^{o}
  t0[1] += t_cam_odo[1];
  ...
  ceres::QuaternionRotatePoint(q_odo_cam, t0, t); // 将里程计坐标系下的点变换到相机坐标系下，c->o * t_{w->c}^{o} = t_{w->c}^{c}
  t[0] = -t[0];
  t[1] = -t[1];
  t[2] = -t[2];
...
class ReprojectionError1 {
 public:
   ...
   ReprojectionError1(Eigen::Vector3d  observed_P, Eigen::Vector2d  observed_p)
   :  m_observed_P(std::move(observed_P)), m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(Eigen::Matrix2d::Identity()) { }

   ReprojectionError1(Eigen::Vector3d  observed_P, Eigen::Vector2d  observed_p, Eigen::Matrix2d  sqrtPrecisionMat)
   :  m_observed_P(std::move(observed_P)), m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(std::move(sqrtPrecisionMat)) { }

   ReprojectionError1(const std::vector<double>& intrinsic_params, Eigen::Vector3d  observed_P, Eigen::Vector2d  observed_p)
   :  m_intrinsic_params(intrinsic_params), m_observed_P(std::move(observed_P)), m_observed_p(std::move(observed_p)) { }
   ...
   template <typename T>
   bool operator()(const T* const q_cam_odo, const T* const t_cam_odo, const T* const p_odo, const T* const att_odo, T* residuals) const {
     T q[4], t[3];
 ...
 private:
   ...
   // square root of precision matrix精度矩阵，也称为信息矩阵，是协方差矩阵的逆
...
class ReprojectionError2 {
 public:
   ...
   ReprojectionError2(const std::vector<double>& intrinsic_params, Eigen::Vector2d  observed_p)
   :  m_intrinsic_params(intrinsic_params), m_observed_p(std::move(observed_p)) { }
...
class ReprojectionError3 {
 public:
   ...
   explicit ReprojectionError3(Eigen::Vector2d  observed_p)
   :  m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(Eigen::Matrix2d::Identity()), m_optimize_cam_odo_z(true) { }

   ReprojectionError3(Eigen::Vector2d  observed_p, Eigen::Matrix2d  sqrtPrecisionMat)
   :  m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(std::move(sqrtPrecisionMat)), m_optimize_cam_odo_z(true) { }

   ReprojectionError3(const std::vector<double>& intrinsic_params, Eigen::Vector2d  observed_p)
   :  m_intrinsic_params(intrinsic_params), m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(Eigen::Matrix2d::Identity()),
      m_optimize_cam_odo_z(true) { }

   ReprojectionError3(const std::vector<double>& intrinsic_params, Eigen::Vector2d  observed_p, Eigen::Matrix2d  sqrtPrecisionMat)
   :  m_intrinsic_params(intrinsic_params), m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(std::move(sqrtPrecisionMat)), m_optimize_cam_odo_z(true) {}


   ReprojectionError3(const std::vector<double>& intrinsic_params, Eigen::Vector3d  odo_pos, Eigen::Vector3d  odo_att,
                      Eigen::Vector2d  observed_p, bool optimize_cam_odo_z)
   :  m_intrinsic_params(intrinsic_params), m_odo_pos(std::move(odo_pos)), m_odo_att(std::move(odo_att)) , m_observed_p(std::move(observed_p)),
      m_optimize_cam_odo_z(optimize_cam_odo_z) {}

   ReprojectionError3(const std::vector<double>& intrinsic_params, const Eigen::Quaterniond& cam_odo_q, Eigen::Vector3d  cam_odo_t,
                      Eigen::Vector3d  odo_pos, Eigen::Vector3d  odo_att, Eigen::Vector2d  observed_p)
   :  m_intrinsic_params(intrinsic_params), m_cam_odo_q(cam_odo_q), m_cam_odo_t(std::move(cam_odo_t)), m_odo_pos(std::move(odo_pos)), m_odo_att(std::move(odo_att)),
      m_observed_p(std::move(observed_p)), m_optimize_cam_odo_z(true) {}

   // variables: camera intrinsics, camera-to-odometry transform,
   //            odometry extrinsics, 3D point
   template <typename T>
   bool operator()(const T* const intrinsic_params, const T* const q_cam_odo, const T* const t_cam_odo,
                   const T* const p_odo, const T* const att_odo, const T* const point, T* residuals) const {
     T q[4], t[3];
   ...
   template <typename T>
   bool operator()(const T* const q_cam_odo, const T* const t_cam_odo, const T* const p_odo, const T* const att_odo,
...
class StereoReprojectionError {
 public:
   ...
   StereoReprojectionError(Eigen::Vector3d  observed_P, Eigen::Vector2d  observed_p_l, Eigen::Vector2d  observed_p_r)
   :  m_observed_P(std::move(observed_P)), m_observed_p_l(std::move(observed_p_l)), m_observed_p_r(std::move(observed_p_r))
   { }

   template <typename T>
   bool operator()(const T* const intrinsic_params_l, const T* const intrinsic_params_r, const T* const q_l,
                   const T* const t_l, const T* const q_l_r, const T* const t_l_r, T* residuals) const {
     Eigen::Matrix<T, 3, 1> P;
...
class ComprehensionError {
 public:
   ...
   ComprehensionError(Eigen::Vector3d  observed_P, Eigen::Vector2d  observed_p)
   :  m_observed_P(std::move(observed_P)), m_observed_p(std::move(observed_p)), m_sqrtPrecisionMat(Eigen::Matrix2d::Identity())
   { }
...
CostFunctionFactory::CostFunctionFactory() = default;

boost::shared_ptr<CostFunctionFactory>
CostFunctionFactory::instance() {
    ...
    case CAMERA_INTRINSICS | CAMERA_POSE: {
      switch (camera->modelType()) {
        case Camera::KANNALA_BRANDT:
          costFunction =  // ceres::AutoDiffCostFunction使用自动微分计算雅可比矩阵，它是一个模板类，模板参数为：自定义的代价函数类，残差的维度，自变量的维度
                          // 用户定义的代价函数类需要包含一个 operator() 方法，该方法计算给定参数的残差。这个方法应该使用模板类型，以便 Ceres 可以使用自动微分技术
        ...
        case Camera::PINHOLE:
          costFunction =
              new ceres::AutoDiffCostFunction<ReprojectionError1<PinholeCamera>, 2, 8, 4, 3>(     // 内参的8个优化变量（包括4个畸变参数），旋转的4个优化变量（四元数），平移的3个优化变量
                  new ReprojectionError1<PinholeCamera>(observed_P, observed_p));                 // ceres::AutoDiffCostFunction的参数是指向代价函数的指针
          break;
        case Camera::MEI:
          costFunction =
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& camera, const Eigen::Vector3d& observed_P, const Eigen::Vector2d& observed_p,
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& camera, const Eigen::Vector2d& observed_p,
                                          int flags, bool optimize_cam_odo_z) {
  ceres::CostFunction* costFunction = nullptr;
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& camera, const Eigen::Vector2d& observed_p,
                                          const Eigen::Matrix2d& sqrtPrecisionMat, int flags, bool optimize_cam_odo_z) {
  ceres::CostFunction* costFunction = nullptr;
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& camera, const Eigen::Vector3d& odo_pos, const Eigen::Vector3d& odo_att,
                                          const Eigen::Vector2d& observed_p, int flags, bool optimize_cam_odo_z) {
  ceres::CostFunction* costFunction = nullptr;
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& camera, const Eigen::Quaterniond& cam_odo_q, const Eigen::Vector3d& cam_odo_t,
                                          const Eigen::Vector3d& odo_pos, const Eigen::Vector3d& odo_att, const Eigen::Vector2d& observed_p, int flags) {
  ceres::CostFunction* costFunction = nullptr;
...
ceres::CostFunction*
CostFunctionFactory::generateCostFunction(const CameraConstPtr& cameraL, const CameraConstPtr& cameraR, const Eigen::Vector3d& observed_P,
                                          const Eigen::Vector2d& observed_p_l, const Eigen::Vector2d& observed_p_r) {
  ceres::CostFunction* costFunction = nullptr;
