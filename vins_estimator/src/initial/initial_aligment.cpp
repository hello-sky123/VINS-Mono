...
/**
 * @brief 求解陀螺仪的零偏，同时利用求出来的重新进行预积分
...
MatrixXd TangentBasis(Vector3d& g0) // g0是重力向量的初始值（方向是之前求得的，大小使用了先验）
{
  Vector3d b, c;
...
{
  Vector3d g0 = g.normalized() * G.norm(); // 重力方向的初始值 * 重力加速度的模（固定的已知值）
  Vector3d lx, ly; // 重力方向的切线基
  //VectorXd x;
  ...
  {
    MatrixXd lxly(3, 2);
    lxly = TangentBasis(g0); // 重力方向的切线基组成的3x2的矩阵
    int i = 0;
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
      ...
      tmp_A.block<3, 2>(0, 6) = 0.5 * frame_i->second.R.transpose() * dt * dt * lxly; //
...
{
  int all_frame_count = (int)all_image_frame.size();
  int n_state = all_frame_count * 3 + 3 + 1; // 所有帧的速度，枢纽帧的重力方向，以及尺度等总的状态量
  ...
  for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
  {
    ...
    MatrixXd r_A = tmp_A.transpose() * cov_inv * tmp_A; // 转换为对称正定矩阵
    VectorXd r_b = tmp_A.transpose() * cov_inv * tmp_b;

    // 将小的矩阵加到大的矩阵中
    A.block<6, 6>(i * 3, i * 3) += r_A.topLeftCorner<6, 6>();
    b.segment<6>(i * 3) += r_b.head<6>(); // 从第i*3个元素开始，取6个元素
