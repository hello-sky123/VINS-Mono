...
:  Camera::Parameters(MEI),
   m_xi(0.0), m_k1(0.0), m_k2(0.0),
   m_p1(0.0), m_p2(0.0), m_gamma1(0.0),
   m_gamma2(0.0), m_u0(0.0), m_v0(0.0)
{ }
...
:  Camera::Parameters(MEI, cameraName, w, h),
   m_xi(xi), m_k1(k1), m_k2(k2),
   m_p1(p1), m_p2(p2), m_gamma1(gamma1),
   m_gamma2(gamma2), m_u0(u0), m_v0(v0)
{ }

double& CataCamera::Parameters::xi() {
  return m_xi;
}

double& CataCamera::Parameters::k1() {
  return m_k1;
}

double& CataCamera::Parameters::k2() {
  return m_k2;
}

double& CataCamera::Parameters::p1() {
  return m_p1;
}

double& CataCamera::Parameters::p2() {
  return m_p2;
}

double& CataCamera::Parameters::gamma1() {
  return m_gamma1;
}

double& CataCamera::Parameters::gamma2() {
  return m_gamma2;
}

double& CataCamera::Parameters::u0() {
  return m_u0;
}

double& CataCamera::Parameters::v0() {
  return m_v0;
}

double CataCamera::Parameters::xi() const {
  return m_xi;
}

double CataCamera::Parameters::k1() const {
  return m_k1;
}

double CataCamera::Parameters::k2() const {
  return m_k2;
}

double CataCamera::Parameters::p1() const {
  return m_p1;
}

double CataCamera::Parameters::p2() const {
  return m_p2;
}

double CataCamera::Parameters::gamma1() const {
  return m_gamma1;
}

double CataCamera::Parameters::gamma2() const {
  return m_gamma2;
}

double CataCamera::Parameters::u0() const {
  return m_u0;
}

double CataCamera::Parameters::v0() const {
  return m_v0;
}
...
bool CataCamera::Parameters::readFromYamlFile(const std::string& filename) {
  ...
  m_xi = static_cast<double>(n["xi"]); // epsilon
...
CataCamera::CataCamera(const std::string& cameraName, int imageWidth, int imageHeight,
                       double xi, double k1, double k2, double p1, double p2,
                       double gamma1, double gamma2, double u0, double v0)
...
int CataCamera::imageWidth() const {
  return mParameters.imageWidth();
}

int CataCamera::imageHeight() const {
...
void CataCamera::estimateIntrinsics(const cv::Size& boardSize,
  ...
  rvecs.assign(objectPoints.size(), cv::Mat()); // assign给容器赋值,用新数据替换原有的数据
  tvecs.assign(objectPoints.size(), cv::Mat()); // 有两个版本，第一个是用一个值重复n次赋值，第二个是迭代器范围内的值赋值
  ...
  for (size_t i = 0; i < imagePoints.size(); ++i) {
    for (int r = 0; r < boardSize.height; ++r) {
      ...
      cv::Mat C;
      cv::SVD::solveZ(P, C); // 求解A.x = 0的最小二乘解，即求解A的最小奇异值对应的奇异向量,x是单位化的
      ...
      if (hypot(nx, ny) > 0.95) { // hypot(x,y) = sqrt(x^2 + y^2)计算斜边长
        continue;
      }
 ...
 * \param p return value, contains the image point coordinates
 */
void CataCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p,
...
cv::Mat CataCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize,
...
int CataCamera::parameterCount() const {
  return 9;
}
