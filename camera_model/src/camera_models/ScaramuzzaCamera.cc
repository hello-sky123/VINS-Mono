...
:  Camera::Parameters(SCARAMUZZA),
   m_C(0.0), m_D(0.0), m_E(0.0), m_center_x(0.0), m_center_y(0.0)
{
...
bool OCAMCamera::Parameters::readFromYamlFile(const std::string& filename) {
  ...
  for(int i = 0; i < SCARAMUZZA_POLY_SIZE; i++)
    m_poly[i] = static_cast<double>(n[std::string("p") + boost::lexical_cast<std::string>(i)]); // boost::lexical_cast可进行字符串和数字之间的转换
...
int OCAMCamera::imageWidth() const {
  return mParameters.imageWidth();
}

int OCAMCamera::imageHeight() const {
                                    ...
                                    const std::vector<std::vector<cv::Point2f>>& imagePoints)
{
  ...
  // i-th image
  for (int image_index = 0; image_index < imagePoints.size(); ++image_index) {
    ...
    Eigen::MatrixXd M(objPts.size(), 6); // 存储外参的矩阵

    // Reference: Page 32 of
    // " Scaramuzza, D. Omnidirectional Vision: from Calibration to Robot Motion Estimation, ETH Zurich. Thesis no. 17635."
    // estimate extrinsic parameters
    for(int corner_index = 0; corner_index < objPts.size(); ++corner_index) {
    ...
    const double sr32_squared_1 = (- (CC - BB) + sqrt(square(CC - BB) + 4.0 * AA)) / 2.0; // 利用正交性和单位化的条件，求解出sr32的平方的两个解
...
// printf("rst = %.12f\n", sr32_squared_1*sr32_squared_1 + (CC-BB)*sr32_squared_1 - AA);
    ...
    for (auto& H : H_values) {
      Eigen::MatrixXd A_mat(2 * imagePoints.at(image_index).size(), 4); // 2n x 4, n is the number of corners
      ...
      for(size_t j = 0; j < imagePoints.at(image_index).size(); ++j) {
        ...
        A_mat(line_index + 0, 1) = A * rou * rou;
        A_mat(line_index + 1, 1) = C * rou * rou;
        A_mat(line_index + 0, 2) = A * rou * rou * rou;
        A_mat(line_index + 1, 2) = C * rou * rou * rou;
...
//void OCAMCamera::initUndistortMap(cv::Mat& map1, cv::Mat& map2, double fScale) const {
...
cv::Mat OCAMCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize,
...
int OCAMCamera::parameterCount() const {
