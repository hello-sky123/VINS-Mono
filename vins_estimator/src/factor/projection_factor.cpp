...
ProjectionFactor::ProjectionFactor(Eigen::Vector3d _pts_i, Eigen::Vector3d _pts_j): pts_i(std::move(_pts_i)), pts_j(std::move(_pts_j))
{
...
bool ProjectionFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
{
  ...
  Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i; // 地图点在第i帧相机坐标系下的
  Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;
  Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;
  Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); // 将点i转换到j的imu坐标系
  Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); // 第j帧相机坐标系
  ...
  residual = sqrt_info * residual; // 置信度认为是一致的1.5个像素
  ...
  if (jacobians)
  {
    ...
    Eigen::Matrix<double, 2, 3> reduce(2, 3); // dr/dp
