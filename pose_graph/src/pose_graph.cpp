...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
  ...
  Vector3d vio_P_cur;
  Matrix3d vio_R_cur;
  if (sequence_cnt != cur_kf->sequence) // 判断sequence是否发生变化
  {
    sequence_cnt++;
    sequence_loop.push_back(false);
    w_t_vio = Eigen::Vector3d(0, 0, 0);
  ...
  if (loop_index != -1) // 找到了有效回环
  {
    ...
    if (cur_kf->findConnection(old_kf)) // 如果确定两帧回环
    {
      ...
      relative_t = cur_kf->getLoopRelativeT(); // 得到回环相对位移
      relative_q = (cur_kf->getLoopRelativeQ()).toRotationMatrix(); // 得到回环相对旋转
      // T_w_cur = T_w_old * R_old_cur这就是回环矫正后当前帧的位姿
      ...
      // 如果这两个不是同一个sequence，且当前sequence没有和之前合并
      if (old_kf->sequence != cur_kf->sequence && sequence_loop[cur_kf->sequence] == 0)
      {
        ...
        sequence_loop[cur_kf->sequence] = true; // 标记当前sequence已经合并
      }
      m_optimize_buf.lock();
      optimize_buf.push(cur_kf->index); // 通知4DoF优化线程进行优化
  ...
  P = r_drift * P + t_drift; // 根据全局优化后进行修正
  R = r_drift * R;
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
  ...
  if (DEBUG_IMAGE)
  {
    int feature_num = (int)keyframe->keypoints.size();
  ...
  if (DEBUG_IMAGE)
  {
    for (auto& i: ret)
    {
      ...
      putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(i.Score), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255));
  ...
  if (!ret.empty() && ret[0].Score > 0.05)
    for (unsigned int i = 1; i < ret.size(); i++)
    {
      ...
      {
        find_loop = true; // 则认为找到了回环
        int tmp_index = (int)ret[i].Id;
        if (DEBUG_IMAGE)
        {
          auto it = image_pool.find(tmp_index);
          cv::Mat tmp_image = (it->second).clone();
  ...
  {
    int min_index = -1;
    // 寻找得分大于0.015的id最小的候选帧，这样可以尽可能调整更多的帧的位姿
    for (auto& i: ret)
    {
      if (min_index == -1 || (i.Id < min_index && i.Score > 0.015))
        min_index = (int)i.Id;
    }
    return min_index; // 当前帧和min_index帧形成了回环
  }
  else
    return -1;
...
void PoseGraph::optimize4DoF()
{
  while (true)
  {
    ...
    if (cur_index != -1)
    {
      ...
      int max_length = cur_index + 1; // 预设一个最大的长度，总之优化帧数不会超过这个数
      ...
      int i = 0;
      // 遍历所有的关键帧，将位姿和yaw角加入到优化问题中
      for (it = keyframelist.begin(); it != keyframelist.end(); it++)
      {
        if ((*it)->index < first_looped_index)
          continue;
        (*it)->local_index = i; // 这个是本次优化的index，从0开始
        Quaterniond tmp_q;
        Matrix3d tmp_r;
        Vector3d tmp_t;
        ...
        if((*it)->has_loop)
        {
          ...
          int connected_index = getKeyFrame((*it)->loop_index)->local_index; // 得到回环帧在本次优化中的index
          Vector3d euler_conncected = Utility::R2ypr(q_array[connected_index].toRotationMatrix());
          Vector3d relative_t;
          relative_t = (*it)->getLoopRelativeT(); // 得到当前帧和回环帧相对位移
...
void PoseGraph::savePoseGraph()
{
  ...
  for (it = keyframelist.begin(); it != keyframelist.end(); it++)
  {
    ...
    if (DEBUG_IMAGE)
    {
      ...
      imwrite(image_path, (*it)->image);
    }
    // 分别存储VIO位姿和全局优化后的位姿以及回环信息
...
void PoseGraph::loadPoseGraph()
{
                ...
                &loop_info_4, &loop_info_5, &loop_info_6, &loop_info_7, &keypoints_num) != EOF)
  {
    ...
    for (int i = 0; i < keypoints_num; i++)
    {
      ...
      tmp_keypoint_norm.pt.y = (float)p_y_norm;
    ...
    auto* keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints, keypoints_norm, brief_descriptors);
...
void PoseGraph::updateKeyFrameLoop(int index, Eigen::Matrix<double, 8, 1>& _loop_info)
{
  ...
  if (abs(_loop_info(7)) < 30.0 && Vector3d(_loop_info(0), _loop_info(1), _loop_info(2)).norm() < 20.0)
  {
    ...
    {
      KeyFrame* old_kf = getKeyFrame(kf->loop_index); // 得到回环帧的位姿
      Vector3d w_P_old, w_P_cur, vio_P_cur;
      Matrix3d w_R_old, w_R_cur, vio_R_cur;
      ...
      // T_w_cur = T_w_loop * T_loop_cur
      w_P_cur = w_R_old * relative_t + w_P_old;
      w_R_cur = w_R_old * relative_q;
      double shift_yaw;
      Matrix3d shift_r;
      Vector3d shift_t;
      // 更新VIO位姿和修正位姿的delta_pose
      shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
      shift_r = Utility::ypr2R(Vector3d(shift_yaw, 0, 0));
      shift_t = w_P_cur - w_R_cur * vio_R_cur.transpose() * vio_P_cur;
