...
#include <set>
#ifdef _WIN32
#include <winsock.h> // windows平台的网络编程接口，可以用于创建TCP/IP客户端和服务器应用程序
#else
#include <ctime>
#endif
...
const double WGS84_A = 6378137.0; // WGS84(world geodetic system)是用于地图、导航和全球定位系统的一个坐标系统
const double WGS84_ECCSQ = 0.00669437999013; // WGS84_A：这是 WGS84椭球体的半长轴长度，单位是米，WGS84_ECCSQ：这是WGS84椭球体的偏心率（eccentricity）的平方
...
double hypot3(double x, double y, double z) // 计算3个数的平方和的平方根
{
...
double d2r(double deg) // 角度转弧度
{
  return deg / 180.0 * M_PI; // M_PI、sqrt在math.h中定义，有两个命名空间，一个是std，一个是global
}
...
double sinc(double theta) // 辛格函数，sin(x)/x
{
...
unsigned long long timeInMicroseconds() // 在64位Linux平台上，long和long long一样都是8个字节
{
  struct timespec tp{}; // timespec是一个POSIX-compliant的结构体，用来记录时间，其中tv_sec是秒，tv_nsec是纳秒,都是long，表示从纪元时间到现在的时间
#ifdef __APPLE__
 tp = orwl_gettime();
#else
  clock_gettime(CLOCK_REALTIME, &tp); // 获取当前时钟，CLOCK_REALTIME表示从1970年1月1日0时0分0秒到现在的时间
#endif
...
{
  {1.0f,0.f,0.f}, // 第二个颜色分量从0开始，每次增加0.007874
  {1.0f,0.007874f,0.f},
  {1.0f,0.015748f,0.f},
  {1.0f,0.023622f,0.f},
  {1.0f,0.031496f,0.f},
...
void colorDepthImage(cv::Mat& imgDepth, cv::Mat& imgColoredDepth,
					 float minRange, float maxRange) // imgDepth输入的深度图像，格式为16UC1，单位是mm，minRange 和 maxRange: 指定深度值的范围，用于归一化和映射到颜色
{
  imgColoredDepth = cv::Mat::zeros(imgDepth.size(), CV_8UC3);

  for (int i = 0; i < imgColoredDepth.rows; ++i)
  {
    const uint16_t* depth = imgDepth.ptr<uint16_t>(i); // 需要注意原来深度图的格式，ptr函数返回某一行开始处的指针
    auto* pixel = imgColoredDepth.ptr<unsigned char>(i);
    for (int j = 0; j < imgColoredDepth.cols; ++j)
    {
      if (depth[j] != 0)
      { // 这部分代码首先将深度值归一化到 [0, 1] 范围，然后将其映射到一个索引 idx，该索引用于查找颜色映射表 colormapJet
        int idx = fminf(depth[j] - minRange, maxRange - minRange) / (maxRange - minRange) * 127.0f;
        idx = 127 - idx;

        pixel[0] = uint8_t(colormapJet[idx][2] * 255.0f);
        pixel[1] = uint8_t(colormapJet[idx][1] * 255.0f);
        pixel[2] = uint8_t(colormapJet[idx][0] * 255.0f); // 这里的颜色顺序是BGR，而colormapJet是RGB
      }
...
bool colormap(const std::string& name, unsigned char idx,     // name表示选择哪个颜色映射，idx表示选里面的哪个颜色
...
std::vector<cv::Point2i> bresLine(int x0, int y0, int x1, int y1)
{
  ...
  int sx = (x0 < x1) ? 1 : -1; // sx和sy是x和y的增量方向，如果x0<x1，那么sx=1，否则sx=-1
  int sy = (y0 < y1) ? 1 : -1;

  int err = dx - dy; // err 是一个累积误差，用于确定下一个像素应该是沿 x 方向、y 方向还是两者都移动

  while (true) // 在每次迭代中，算法都会确定下一个要绘制的像素，并将其添加到 cells 向量中
  {
...
char UTMLetterDesignator(double latitude) // 给定纬度，返回UTM（地图投影系统）字母
{ // 它将地球表面划分为一系列 8° 纬度宽的区域，每个区域都由一个字母来标识
