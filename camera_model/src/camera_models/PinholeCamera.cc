...
:  Camera::Parameters(PINHOLE),
   m_k1(0.0), m_k2(0.0), m_p1(0.0), m_p2(0.0),
   m_fx(0.0), m_fy(0.0), m_cx(0.0), m_cy(0.0)
{ }

PinholeCamera::Parameters::Parameters(const std::string& cameraName, int w, int h,
...
double PinholeCamera::Parameters::k1() const {
  return m_k1;
}

double PinholeCamera::Parameters::k2() const {
  return m_k2;
}

double PinholeCamera::Parameters::p1() const {
  return m_p1;
}

double PinholeCamera::Parameters::p2() const {
  return m_p2;
}

double PinholeCamera::Parameters::fx() const {
  return m_fx;
}

double PinholeCamera::Parameters::fy() const {
  return m_fy;
}

double PinholeCamera::Parameters::cx() const {
  return m_cx;
}

double PinholeCamera::Parameters::cy() const {
  return m_cy;
}
...
PinholeCamera::PinholeCamera(const std::string& cameraName, int imageWidth, int imageHeight,
                             double k1, double k2, double p1, double p2, double fx, double fy, double cx, double cy)
...
Camera::ModelType PinholeCamera::modelType() const {
  return mParameters.modelType();
}

const std::string& PinholeCamera::cameraName() const {
  return mParameters.cameraName();
}

int PinholeCamera::imageWidth() const {
  return mParameters.imageWidth();
}

int PinholeCamera::imageHeight() const {
...
void PinholeCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const {
  ...
  if (m_noDistortion) {
    ...
    int n = 8;
    Eigen::Vector2d d_u;
    // distortion函数可以计算出畸变后的坐标与畸变前的坐标的差值（输入的第一个参数应该是未畸变的归一化坐标）
    // 由于这时还不知道未畸变的归一化坐标，所以这里用的是畸变后的归一化坐标代替
 ...
 *
 * \param P 3D point coordinates （相机坐标系）
...
/**
 * 计算同样大小的图像的畸变校正后的像素坐标
 */
void PinholeCamera::initUndistortMap(cv::Mat& map1, cv::Mat& map2, double fScale) const {
  cv::Size imageSize(mParameters.imageWidth(), mParameters.imageHeight());

  cv::Mat mapX = cv::Mat::zeros(imageSize, CV_32F); // 创建和原图一样大小的mapX和mapY
  cv::Mat mapY = cv::Mat::zeros(imageSize, CV_32F); // mapX和mapY分别是畸变后的对应位置的像素坐标u, v

  // 遍历每一个像素，计算畸变后的像素坐标
  for (int v = 0; v < imageSize.height; ++v) { // 行
    for (int u = 0; u < imageSize.width; ++u) { // 列
      double mx_u = m_inv_K11 / fScale * u + m_inv_K13 / fScale; // fScale是缩放因子
  ...
  // 输入只能是1通道或者2通道的float以及1通道或者2通道的16位整型，如果是最近邻插值，输入就只能是整型
  // 将图像的一种表示方式转换为另一种表示方式
  cv::convertMaps(mapX, mapY, map1, map2, CV_32FC1, false);
}

cv::Mat PinholeCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize,
  ...
  cv::cv2eigen(rmat, R); // 将旋转矩阵转换为Eigen形式的矩阵
  R_inv = R.inverse();
...
int PinholeCamera::parameterCount() const {
  return 8;
}
...
std::string
PinholeCamera::parametersToString() const {
