...
#define MAX_CONTOUR_APPROX  7 // 轮廓近似的像素点数目
...
void Chessboard::findCorners(bool useOpenCV) // 检测棋盘格角点，useOpenCV表示是否使用OpenCV自带的函数
{                                       // patternSize棋盘格的尺寸（内部角点数）。例如，对于一个8x8的棋盘格，这个尺寸应该是7x7（因为角点数量比方格数量少1）
  mCornersFound = findChessboardCorners(mImage, mBoardSize, mCorners,
                                        CV_CALIB_CB_ADAPTIVE_THRESH +  // 使用自适应阈值来将灰度图像转换为二值图像，这对于由于光线不均匀导致的图像亮度变化很有用
                                        CV_CALIB_CB_NORMALIZE_IMAGE +       // 是否使用直方图均衡化来对图像进行预处理来增加对比度
                                        CV_CALIB_CB_FILTER_QUADS +          // 对检测到的四边形进行额外的滤波，去除一些不合格的四边形
                                        CV_CALIB_CB_FAST_CHECK,             // 进行一个快速检测，如果图像不符合棋盘格的规则，则快速拒绝掉
                                        useOpenCV);
...
bool Chessboard::cornersFound() const
{
  return mCornersFound;
}

const cv::Mat& Chessboard::getImage() const // 返回待检测棋盘格角点的灰度图像
{
  return mImage;
}

const cv::Mat& Chessboard::getSketch() const
{
  return mSketch;
}
...
bool Chessboard::findChessboardCornersImproved(const cv::Mat& image, const cv::Size& patternSize,
                                               std::vector<cv::Point2f>& corners, int flags)
{
  ...
  //int bestDilation        = -1;
  const int minDilations    =  0; // 最小膨胀像素数
  const int maxDilations    =  7;

  std::vector<ChessboardQuadPtr> outputQuadGroup;

  if (image.depth() != CV_8U || image.channels() == 2) // depth表示底层数据的数据类型
  {
    return false;
  }
  ...
  cv::Mat img = image; // 传入的引用复制拷贝后是相互独立的，即修改img不会影响image

  // Image histogram normalization and
  // BGR to Grayscale image conversion (if applicable)
  // MARTIN: Set to "false"
  if (image.channels() != 1 || (flags & CV_CALIB_CB_NORMALIZE_IMAGE)) // 只要输入图像是彩色图或者有CV_CALIB_CB_NORMALIZE_IMAGE，就执行if
  {
    cv::Mat norm_img(image.rows, image.cols, CV_8UC1);

    if (image.channels() != 1) // 如果输入图像是彩色图
    {
      cv::cvtColor(image, norm_img, cv::COLOR_BGR2GRAY); // 将输入图像转换为灰度图
      img = norm_img;
    }
...
bool Chessboard::checkChessboard(const cv::Mat& image, const cv::Size& patternSize) const
{
  ...
  // kernel膨胀核（取cv::Mat()时是一个3x3的矩形核），anchor锚点（从这个参数开始参数都有默认值，cv::Point(-1,-1)表示锚点在元素的中心位置），iterations腐蚀迭代次数
